...
function actualGmRateFromInputs() {
  ...
  const effectiveBill = baseBillRate * (1 - (volumeApplies ? requestedDiscount : 0));
  const cost = loadedCost(workerType, payRate);
  return effectiveBill > 0 ? (effectiveBill - cost) / effectiveBill : 0;
}

function syncTargetGmToActual() {
  const actualGm = Math.min(Math.max(actualGmRateFromInputs(), 0), 0.95);
  fields.targetGm.value = (actualGm * 100).toFixed(1);
}
...
form.addEventListener("input", (event) => {
  if (event.target.matches("#payRate, #billRate")) {
    syncTargetGmToActual();
  }
  calculate();
});
